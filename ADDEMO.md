...
- ❌ 不要把 `index.html` 里 `assets/hub.{css,js}?v=__BUILD__` 的占位符改成固定版本号
  或直接删掉 —— 它会由部署流程换成当次提交的 SHA。写死一个不变的版本号，等于回到
  「缓存里新旧混用」，页头就是因为这个坏过一次（详见第 5 节）。
...
`index.html` 里引 `assets/hub.css` / `assets/hub.js` 时跟了一个占位符：

```html
<link rel="stylesheet" href="assets/hub.css?v=__BUILD__">
<script src="assets/hub.js?v=__BUILD__" defer></script>
```

`__BUILD__` 由 `pages.yml` 的「给静态资源打上版本号」那一步换成当次提交的短 SHA
（本地直接打开时它就是个普通查询串，无害）。

**为什么非得这么做**：GitHub Pages 给静态文件的缓存头大约 10 分钟。URL 如果一直不变，
改版后浏览器或 CDN 就会把「新的 `index.html` / `hub.js`」和「旧的 `hub.css`」拼着用 ——
页头真的因此坏过一次：计数行被标签行压住看不清、搜索框被撑成整页宽的长条。
每次部署换一个新 URL，页面和它引用的资源就永远配对。

所以：**别把这个占位符改成固定版本号**（`?v=1.0.0` 这种），也别为了「干净」把它删掉 ——
两者都会让上面那个问题原样复发。

> 唯一的例外是主站首页那条引用：它指向别的仓库，没法自动跟着版本走。所以**改完
> `hub.css` 后，主站首页最多 10 分钟内可能还是旧样式**，在自己的浏览器上按一次
> `Ctrl + F5`（强制刷新）即可，不用管它。
...
- [ ] 窄窗口（手机宽度）下卡片和查看器都正常
- [ ] `index.html` 里资源引用仍是 `?v=__BUILD__`（没被改成固定版本号，也没被删掉）
