...
4. **`index.html` 里资源引用必须保持 `?v=__BUILD__` 这个占位符**（`pages.yml` 会在部署时
   把它换成当次提交的短 SHA）。别改成 `?v=1.0.0` 这类固定版本号，也别删掉：GitHub Pages
   有约 10 分钟缓存，URL 不变就会让「新页面 + 旧样式」混用 —— 页头已经因此坏过一次。
   详见 ADDEMO.md 第 5 节「资源为什么带 `?v=__BUILD__`」。
